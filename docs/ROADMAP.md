...
- ✅ **Destination/route validation** (2026-08-04): one Overpass query checks all 8 candidate bearings for urban drivable streets (excludes motorways, tracks/paper roads, service ways, private access, unpaved) and snaps the destination onto the nearest one; sea bearings simply have no streets and are never picked. Also applies when a destination is reached mid-session (fresh validated destination instead of re-routing to the same point). Overpass down → pre-MVP-1 raw random bearing.
