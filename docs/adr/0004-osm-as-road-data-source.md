...
- **Decision:** At route-build time, query OSM Overpass for a corridor along the route polyline and prefetch: `maxspeed`, `highway=stop`, `railway=level_crossing`, `crossing=*`, and roundabout geometry. The session engine consumes these as typed control points with coordinates; monitoring matches by proximity to the GPS fix, not by instruction text.
- **Consequences:**
